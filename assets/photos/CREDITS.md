...
이 폴더의 사진은 전부 위키미디어 커먼즈에서 받은 퍼블릭 도메인 또는 CC 라이선스 이미지다.
로이터·AP·게티 등 보도사진(wire photo)은 사용하지 않는다. CC BY / CC BY-SA 사진은
인스타그램 캡션 또는 첫 댓글에 출처 표기(아래 "캡션 표기" 문구)가 **필수**다.

| 파일 | 피사체 | 라이선스 | 저작자 | 출처 |
|---|---|---|---|---|
| `powell.jpg` | 제롬 파월 (연준 의장) | Public Domain (US Federal Reserve) | Federal Reserve | [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Jerome_H._Powell,_Federal_Reserve_Chair_(cropped).jpg) |
| `lee_jaeyong.jpg` | 이재용 (삼성전자 회장) | CC BY-SA 3.0 | LEEJAEY68 | [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:이재용삼성총수.jpg) |
...
1. 뉴스에 등장하는 인물(CEO·의장 등)을 최우선으로 사용. 해당 보도 사진이 아니어도
   무방하나 반드시 무료 라이선스(PD/CC0/CC BY/CC BY-SA)일 것.
2. 특정 인물로 대표하기 어려운 뉴스(지수·유가 등)는 관련 건물·시설·제품(회사 로고가
   보이는 기체·건물 등) 사진으로 대체.
3. 라이선스는 다운로드 전에 Wikimedia API(`action=query&prop=imageinfo`)로 직접 조회해
   `LicenseShortName` / `AttributionRequired` 값을 확인한다. 검색 요약만 보고 판단하지 않는다.
